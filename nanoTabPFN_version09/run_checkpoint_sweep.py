...
def _get_run_paths() -> Dict[str, Path]:
    """Return key paths for the W&B run and evaluation_ddim500 outputs."""
    # Adjust the run id below as needed when sweeping a different run.
    run_dir = HERE / "wandb" / "temp"
    files_dir = run_dir / "files"
    eval_dir = run_dir / "evaluation_ddim500"
...
def _build_classifier(ckpt_path: Path, device: torch.device) -> NanoTabPFNClassifier:
    ...
    # Use evaluation_ddim500 settings from the training config if present, but override
...
def main() -> None:
    ...
    for ckpt_path in ckpts:
        ...
        # Flush JSON logs after each checkpoint evaluation_ddim500.
