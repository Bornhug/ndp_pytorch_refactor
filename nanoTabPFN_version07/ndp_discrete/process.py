...
class D3PM:
    ...
    def __init__(
        self,
        model: Callable[[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor | None], torch.Tensor],
    ...
    def loss(
        self,
        y_tokens: torch.Tensor,   # [B,N]
        x_feats: torch.Tensor,    # [B,N,F]
        ...
        *,
        ...
        def model_fn(x_tokens: torch.Tensor, t_in: torch.Tensor) -> torch.Tensor:
            ...
            return self.model(t_float, y_onehot, x_feats, mask)
    ...
    def conditional_sample(
        ...
        *,
        ...
        """
        RePaint-style conditional sampling that keeps context labels clamped.
        """
        ...
        if num_sample_steps is None or num_sample_steps >= T:
            t_schedule = list(range(T - 1, -1, -1))  # t indices in [T-1..0]
        else:
            lin = torch.linspace(T - 1, 0, int(num_sample_steps), dtype=torch.long)
        ...
        for idx, t_int in iterator:
            ...
            for _ in range(num_inner_steps):
                ...
                t_for_sample = t.clamp(min=0, max=self.T - 1)
                if t_int > 0:
                    y_ctx_t = self.core.q_sample(y_ctx_tokens, t_for_sample, generator=None)
                else:
                    y_ctx_t = y_ctx_tokens

                # (2) reverse step on context+target
                y_ctx_oh = torch.nn.functional.one_hot(y_ctx_t, num_classes=V).to(dtype=dtype)
                y_tgt_oh = torch.nn.functional.one_hot(y_tokens, num_classes=V).to(dtype=dtype)
                ...
                logits = self.model(t_tensor, y_aug, x_aug, mask=mask_aug)
                ...
                if idx + 1 < len(t_schedule):
                    ...
                    next_t = max(t_int - 1, 0)
                if next_t >= 0:
                    Qt_next = self.core.q_onestep_mats[next_t]  # [K,K]
                    probs_fwd = Qt_next[y_tokens]               # [B,N,V]
                    y_tokens = torch.multinomial(probs_fwd.reshape(-1, V), 1).reshape(B_tgt, N_tgt)

        return torch.nn.functional.one_hot(y_tokens, num_classes=V).to(dtype=dtype)
...
def loss(
    ...
    *,
    ...
    return process.loss(y_tokens, x_all, mask=mask_all, generator=key)
