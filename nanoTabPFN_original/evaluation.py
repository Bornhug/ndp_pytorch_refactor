...
from nanoTabPFN_original.inference import ddpm_sample, ddim_sample
...
class NanoTabPFNClassifier:
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray) -> NanoTabPFNClassifier:
        ...
        self.y_train_oh = np.eye(self.num_classes, dtype=np.float32)[self.y_train]
    ...
    def predict_proba(self, X_test: np.ndarray, *, desc: str | None = None) -> np.ndarray:
        ...
        """
        if self.X_train is None or self.y_train_oh is None:
            raise RuntimeError("Must call fit() before predict_proba()")
        
        # Convert to tensors
        x_train = torch.from_numpy(self.X_train).to(self.device)
        y_train = torch.from_numpy(self.y_train_oh).to(self.device)
        ...
        # Convert continuous predictions to probabilities via softmax
...
def evaluate(
    ...
    *,
    ...
    for name, X_train, X_test, y_train, y_test in datasets:
        ...
        y_proba = classifier.predict_proba(X_test, desc=desc)
...
def load_checkpoint(checkpoint_path: Path, device: torch.device):
    ...
    """
    from nanoTabPFN_original.train import NanoTabPFNModel
    from neural_diffusion_process_original.process import cosine_schedule, GaussianDiffusion
