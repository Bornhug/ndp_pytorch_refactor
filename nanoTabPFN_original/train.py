...
# Make the project root importable so we can access neural_diffusion_process_original.
import sys
...
from neural_diffusion_process_original.model import BiDimensionalAttentionModel  # noqa: E402
from neural_diffusion_process_original.process import (  # noqa: E402
    GaussianDiffusion,
    cosine_schedule,
    loss as diffusion_loss,
)
from neural_diffusion_process_original.types import Batch  # noqa: E402
from nanoTabPFN_original.config import Config  # noqa: E402
from nanoTabPFN_original.data import PriorDumpDataLoader  # noqa: E402
...
class NanoTabPFNModel(nn.Module):
    ...
    be used with `neural_diffusion_process_original.process.loss`.
    """
    ...
    def _compute_normalization_stats(
        ...
        """
        Compute per-task normalization statistics from all provided inputs.
        """
        if x is None:
            return None, None
    ...
    def forward(
        ...
        *,
        ...
        """
        EpsModel-compatible forward used by neural_diffusion_process_original.

        Args:
            t: [B] (or scalar) timesteps
            y: [B, N, C] or [N, C] noisy labels
            x: [B, N, F] or [N, F] inputs
            mask: Optional [B, N] indicator (ignored, kept for API alignment)
        """
        ...
        mean, std = self._compute_normalization_stats(x)
        x_norm = self._normalize(x, mean, std)

        mask_tensor = None
        if mask is not None:
            mask_tensor = mask.to(device=x.device, dtype=torch.float32)
            if mask_tensor.ndim == 1:
                mask_tensor = mask_tensor.unsqueeze(0)
            elif mask_tensor.ndim > 2:
                raise ValueError(f"Unsupported mask shape: {mask_tensor.shape}")
            if mask_tensor.size(0) != x_norm.size(0):
                mask_tensor = mask_tensor.expand(x_norm.size(0), -1)

        out = self.core(
            x_norm,
            y,
            t.to(x.device, dtype=torch.float32),
            mask_tensor,
        )
    ...
    for step, batch in progress:
        ...
        if not torch.isfinite(loss) or (step <= 3):
            ...
            with torch.no_grad():
                ...
                t_b = t.view(B, 1, 1)
                debug_key = torch.Generator(device=device)
                seed = torch.randint(0, 2**31 - 1, (1,), device=torch.device("cpu")).item()
                debug_key.manual_seed(seed)
                y_t, eps_true = process.forward(debug_key, batch_t.y_target, t_b)
...
def main() -> None:
    ...
    from nanoTabPFN_original.data import find_corrupted_tasks
