...
class CategoricalDiffusionTorch:
    ...
    def _get_transition_mat_uniform(self, t: int) -> torch.Tensor:
        ...
        """
        # if not None: Creates a banded (local) transition matrix: you can only jump to nearby states (like a small random walk)
    ...
    def q_probs(self, x_start: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        B = x_start.shape[0]
        K = self.num_classes
        t = t.to(device=self.device, dtype=torch.long) # shape:[B]
...
class D3PM:
    ...
    def __init__(
        ...
        *,
        model_prediction: ModelPrediction = "x_prev",
        loss_type: LossType = "hybrid",
        hybrid_coeff: float = 1.0,
        transition_mat_type: TransitionMatType = "uniform",
    ) -> None:
    ...
    def loss(
        ...
        *,
        ...
        def model_fn(y_tokens: torch.Tensor, t_in: torch.Tensor) -> torch.Tensor:
            y_onehot = torch.nn.functional.one_hot(y_tokens, num_classes=self.V).to(dtype=x_target.dtype)
    ...
    # Unused at the moment, should fill it up and replace the diffusion_sample() in evaluation_ddim500.py
    @torch.no_grad()
    def sample(
        self,
        ...
        *,
        ...
        def model_fn(y_tokens: torch.Tensor, t_in: torch.Tensor) -> torch.Tensor:
            y_onehot = torch.nn.functional.one_hot(y_tokens, num_classes=self.V).to(dtype=x_feats.dtype)
