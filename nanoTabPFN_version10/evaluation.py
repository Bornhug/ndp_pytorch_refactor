...
Provides sklearn-compatible interface and evaluation_ddim500 on benchmark datasets.
"""
...
from inference import direct_logits, diffusion_sample
...
class NanoTabPFNClassifier:
    ...
    Uses direct conditional logits (no RePaint sampling) for inference.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            sampling_method: "direct" (single pass), "ddpm" (full or subsampled), or "ddim" (subset)
            num_sampling_steps: Number of steps for DDIM/DDPM (ignored for direct)
        ...
        self.eval_num_classes: int | None = None
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray) -> NanoTabPFNClassifier:
        ...
        if num_classes > self.num_classes:
            raise ValueError(
                f"num_classes={num_classes} exceeds model num_outputs={self.num_classes}"
            )
        self.eval_num_classes = num_classes
        
        return self
    
    
    def predict_logits(self, X_test: np.ndarray, *, desc: str | None = None, log_json_path: str | None = None, log_label=None) -> np.ndarray:
        """
        ...
        with torch.no_grad():
            model_prediction = getattr(self.process.core, "model_prediction", "x_start")
            if self.sampling_method == "direct":
                if model_prediction == "x_prev":
                    raise ValueError("sampling_method='direct' is not valid for model_prediction='x_prev'.")
                y_pred = direct_logits(
                    self.model,
                    self.process,
                    x_test,
                    x_train,
            ...
            elif self.sampling_method in {"ddpm", "ddim"}:
                y_pred = diffusion_sample(
                    self.model,
                    self.process,
                    x_test,
                    x_train,
    ...
    def logits_to_proba(self, y_pred: np.ndarray) -> np.ndarray:
        ...
        """
        effective_classes = self.eval_num_classes or self.num_classes
        logits = torch.from_numpy(y_pred)
        if logits.ndim == 1:
            logits = logits.unsqueeze(-1)

        # Drop absorbing channel if present
        if logits.shape[-1] > effective_classes:
            logits = logits[..., : effective_classes]
        ...
        class_idx = logits.squeeze(-1).round().clamp(0, effective_classes - 1).long()
        proba = torch.zeros(logits.shape[0], effective_classes, dtype=torch.float32)
...
def _load_tabarena_diabetes_dataset(
    ...
    used in the official nanoTabPFN evaluation_ddim500 notebook.
    """
...
def load_benchmark_datasets(
    ...
    """
    Load benchmark datasets for evaluation_ddim500.
...
def sample_prior_tasks(
    ...
    """
    Sample evaluation_ddim500 tasks directly from the training H5 prior dump.
    
    Args:
        h5_path: Path to the H5 file used for pretraining
        num_tasks: Number of tasks to sample for evaluation_ddim500
...
def compute_metrics(
    ...
    """
    Compute evaluation_ddim500 metrics.
...
def main():
    """Main evaluation_ddim500 script."""
    import argparse
    ...
    parser.add_argument(
        ...
        help="Inference method: direct (single pass), ddpm (full or subsampled), or ddim (subset)",
    )
    parser.add_argument(
        "--num-sampling-steps",
        type=int,
    ...
    parser.add_argument(
        ...
        const="",
        help=(
            "Comma-separated list of evaluation_ddim500 datasets. "
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the training H5 file for meta-evaluation_ddim500",
    )
    parser.add_argument(
        "--prior-num-tasks",
        type=int,
    ...
    if args.sampling_method == "direct":
        print(f"Inference method: {args.sampling_method} (single forward, no reverse chain)")
    else:
        print(f"Inference method: {args.sampling_method} (iterative reverse diffusion)")
