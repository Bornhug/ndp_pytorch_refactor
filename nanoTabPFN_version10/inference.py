...
def direct_logits(
    ...
    *,
    ...
    """
    Single-step forward pass to obtain model logits conditioned on context.
    For model_prediction='x_prev', logits correspond to x_{t-1}; use t_value=0 for x_start.
...
def diffusion_sample(
    ...
    *,
    ...
    V = process.V
    T = process.T
    model_prediction = getattr(process.core, "model_prediction", "x_start")
    use_xprev = model_prediction == "x_prev"
    ...
    for step_idx, t_int in iterator:
        ...
        if use_xprev:
            logits_xprev = logits_pred
            p_xprev = torch.softmax(logits_xprev, dim=-1)  # [1,N,V]
        else:
            logits_x0 = logits_pred
        ...
        if log_steps is not None:
            ...
            if use_xprev:
                row["logits_xprev"] = logits_xprev[0].detach().cpu().numpy().tolist()
            else:
                row["logits_x0"] = logits_x0[0].detach().cpu().numpy().tolist()
                row["p_x0"] = p_x0[0].detach().cpu().numpy().tolist()
