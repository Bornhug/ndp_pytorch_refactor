...
def train(
    ...
    *,
    ...
    def should_save_checkpoint(step_idx: int) -> bool:
        if step_idx < 10_000:
            return step_idx % 2_500 == 0
        return checkpoint_interval > 0 and step_idx % checkpoint_interval == 0
...
def main() -> None:
    ...
    model_prediction = getattr(config.diffusion, "model_prediction", "x_start")
    if model_prediction == "x_prev" and loss_type in ("cross_entropy_x_start", "hybrid"):
        raise ValueError(
            f"model_prediction='x_prev' only supports loss_type='kl' (got {loss_type})."
        )
