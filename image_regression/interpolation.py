...
def sample_conditional(state: TrainingState, key: Rng):
    ...
    net_with_params = partial(net, state.params_ema)
    y0 = process.conditional_sample(key, x, mask=None, x_context=xc, y_context=yc, mask_context=None,
...
actions = [
    ...
    actions.PeriodicCallback(
        ...
        callback_fn=lambda step, t, **kwargs: writer.write_figures(step,
                                                                   plot_prior_image(kwargs["state"], kwargs["key"]))
    ),
    actions.PeriodicCallback(
...
for i, percentage in enumerate(PERCENTAGES):
    ...
    num_context_points = jnp.where(context_mask == KEEP, jnp.ones_like(context_mask),
                                   jnp.zeros_like(context_mask)).sum()
